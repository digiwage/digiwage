...
"""Test digiwaged with different proxy configuration.

Test plan:
- Start digiwaged's with different proxy configurations
- Use addnode to initiate connections
- Verify that proxies are connected to, and the right connection command is given
- Proxy configurations to test on digiwaged side:
...
- Create digiwageds that connect to them
- Manipulate the digiwageds using addnode (onetry) an observe effects
...
class ProxyTest(PivxTestFramework):
    ...
    def node_test(self, node, proxies, auth, test_onion=True):
        ...
        # Note: digiwaged's SOCKS5 implementation only sends atyp DOMAINNAME, even if connecting directly to IPv4/IPv6
