...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_wage):
        # verify that nodes have the expected WAGE supply
        wage_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(wage_supply, [DecimalAmt(expected_wage)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that WAGE supply was properly updated after the reorgs (including burned fee)
        self.log.info("Check WAGE supply...")
