...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/DIGIWAGEParams"
    else
        PARAMS_DIR="$HOME/.digiwage-params"
    fi
fi
...
Failed to fetch the DIGIWAGE zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
DIGIWAGE - fetch-params.sh

This script will fetch the DIGIWAGE zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common DIGIWAGE zkSNARK parameters. Note that it is
