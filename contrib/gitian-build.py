...
def build():
    ...
    if args.linux:
        ...
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'digiwage='+args.commit, '--url', 'digiwage='+args.url, '../digiwage/contrib/gitian-descriptors/gitian-linux.yml'])
...
def sign():
    ...
    if args.commit_files:
        os.chdir('gitian.sigs')
        commit = False
        if os.path.isfile(args.version+'-win-signed/'+args.signer+'/digiwage-win-signer-build.assert.sig'):
            subprocess.check_call(['git', 'add', args.version+'-win-signed/'+args.signer])
            commit = True
        if os.path.isfile(args.version+'-osx-signed/'+args.signer+'/digiwage-dmg-signer-build.assert.sig'):
...
def main():
    ...
    if args.pull:
        ...
        if not os.path.isdir('../gitian-builder/inputs/digiwage'):
            os.makedirs('../gitian-builder/inputs/digiwage')
