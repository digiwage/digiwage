# Bootstrappable DigiWage Core Builds

This directory contains the files necessary to perform bootstrappable DigiWage
Core builds.
...
To build DigiWage Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      DigiWage Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/digiwage-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/digiwageproject/guix.sigs).
