...
namespace digiwagetxconverter_tests {
//Tests data
...
void checkResult(bool isCreation, std::vector<DigiWageTransaction> results, uint256 hash){
...
void runTest(Chainstate& chainstate, CTxMemPool& mempool, bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    DigiWageTxConverter converter(transaction, chainstate, &mempool, NULL);
    ExtractDigiWageTX digiwageTx;
    BOOST_CHECK(converter.extractionDigiWageTransactions(digiwageTx));
    std::vector<DigiWageTransaction> result = digiwageTx.first;
    if(script2 == CScript()){
...
using namespace digiwagetxconverter_tests;

BOOST_FIXTURE_TEST_SUITE(digiwagetxconverter_tests, TestingSetup)
