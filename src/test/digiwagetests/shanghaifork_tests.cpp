...
void checkChainid(dev::h256& hashTx, ChainstateManager& chainman, const std::string& chain, const digiwageutils::ChainIdType& chainId)
{
    // Select params
    ...
    txs.push_back(createDigiWageTransaction(getCode(CodeID::chainIdcontract), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...

    {
        ...
        std::vector<DigiWageTransaction> txShanghai;
        txShanghai.push_back(createDigiWageTransaction(getCode(CodeID::getChainId), 0, GASLIMIT, dev::u256(1), ++hashTx, proxy));
...
{
    std::vector<DigiWageTransaction> txs;
    txs.push_back(createDigiWageTransaction(getCode(codeId), 0, GASLIMIT, dev::u256(1), ++hashTx, dev::Address()));
...
BOOST_AUTO_TEST_CASE(checking_shanghai_after_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::MAIN, digiwageutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::TESTNET, digiwageutils::ChainIdType::TESTNET);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::REGTEST, digiwageutils::ChainIdType::REGTEST);
...
BOOST_AUTO_TEST_CASE(checking_shanghai_before_fork){
    ...
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::MAIN, digiwageutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::TESTNET, digiwageutils::ChainIdType::MAIN);
    checkChainid(hashTx, *m_node.chainman, CBaseChainParams::REGTEST, digiwageutils::ChainIdType::MAIN);
