#ifndef DIGIWAGETESTS_TEST_UTILS_H
#define DIGIWAGETESTS_TEST_UTILS_H
...
inline void initState(){
    ...
    const std::string dirDigiWage = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<DigiWageState>(new DigiWageState(dev::u256(0), DigiWageState::openDB(dirDigiWage, hashDB, dev::WithExisting::Trust), dirDigiWage + "/digiwageDB", dev::eth::BaseState::Empty));
...
inline dev::Address createDigiWageAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline DigiWageTransaction createDigiWageTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    DigiWageTransaction txEth;
    if(recipient == dev::Address()){
        txEth = DigiWageTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = DigiWageTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<DigiWageTransaction> txs, ChainstateManager& chainman){
    CBlock block(generateBlock());
    DigiWageDGP digiwageDGP(globalState.get(), chainman.ActiveChainstate(), fGettingValuesDGP);
    uint64_t blockGasLimit = digiwageDGP.getBlockGasLimit(chainman.ActiveChain().Tip()->nHeight + 1);
...
#endif // DIGIWAGETESTS_TEST_UTILS_H
