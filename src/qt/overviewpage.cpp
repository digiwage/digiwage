...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeDigiwageAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeDigiwageAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeDigiwageAmount = strAnonymizeDigiwageAmount.remove(strAnonymizeDigiwageAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigiwageAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeDigiwageAmount * COIN) nMaxToAnonymize = nAnonymizeDigiwageAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeDigiwageAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeDigiwageAmount));
        strAnonymizeDigiwageAmount = strAnonymizeDigiwageAmount.remove(strAnonymizeDigiwageAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeDigiwageAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
