...
#define paternVersion "digiwage-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

DigiWageVersionChecker::DigiWageVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, 0);
}

DigiWageVersionChecker::~DigiWageVersionChecker()
{

}

bool DigiWageVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> DigiWageVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(DIGIWAGE_RELEASES)));
    QTimer timer;
...
Version DigiWageVersionChecker::getMaxReleaseVersion()
{
