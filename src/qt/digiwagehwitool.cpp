...
class DigiWageHwiToolPriv
{
public:
    DigiWageHwiToolPriv(QObject *parent)
    {
...
DigiWageHwiTool::DigiWageHwiTool(QObject *parent) : QObject(parent)
{
    d = new DigiWageHwiToolPriv(this);
}

DigiWageHwiTool::~DigiWageHwiTool()
{
    delete d;
}

bool DigiWageHwiTool::enumerate(QList<HWDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    devices.clear();
    std::vector<LedgerDevice> vecDevices;
    if(DigiWageLedger::instance().enumerate(vecDevices, stake))
    {
    ...
    {
        d->strError = QString::fromStdString(DigiWageLedger::instance().errorMessage());
    }

    return devices.size() > 0;
}

bool DigiWageHwiTool::isConnected(const QString &fingerprint, bool stake)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    bool ret = DigiWageLedger::instance().isConnected(strFingerprint, stake);
    if(!ret) d->strError = QString::fromStdString(DigiWageLedger::instance().errorMessage());
    return ret;
}

bool DigiWageHwiTool::getKeyPool(const QString &fingerprint, int type, const QString& path, bool internal, QString &desc)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = DigiWageLedger::instance().getKeyPool(strFingerprint, type, strPath, internal, d->from, d->to, isDescriptorWallet(), strDesc);
...
bool DigiWageHwiTool::getKeyPool(const QString &fingerprint, int type, const QString &path, QStringList &descs)
{
    LOCK(cs_ledger);
    bool ret = true;
    QString desc;
...
bool DigiWageHwiTool::getKeyPoolPKH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::LEGACY, path, descs);
}

bool DigiWageHwiTool::getKeyPoolP2SH(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::P2SH_SEGWIT, path, descs);
}

bool DigiWageHwiTool::getKeyPoolBech32(const QString &fingerprint, const QString& path, QStringList &descs)
{
    return getKeyPool(fingerprint, (int)OutputType::BECH32, path, descs);
}

bool DigiWageHwiTool::signTx(const QString &fingerprint, QString &psbt)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strPsbt = psbt.toStdString();
    bool ret = DigiWageLedger::instance().signTx(strFingerprint, strPsbt);
    psbt = QString::fromStdString(strPsbt);
    if(!ret) d->strError = QString::fromStdString(DigiWageLedger::instance().errorMessage());
    return ret;
}

bool DigiWageHwiTool::signMessage(const QString &fingerprint, const QString &message, const QString &path, QString &signature)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    ...
    bool ret = DigiWageLedger::instance().signMessage(strFingerprint, strMessage, strPath, strSignature);
    signature = QString::fromStdString(strSignature);
    if(!ret) d->strError = QString::fromStdString(DigiWageLedger::instance().errorMessage());
    return ret;
}

bool DigiWageHwiTool::signDelegate(const QString &fingerprint, QString &psbt)
{
...
bool DigiWageHwiTool::displayAddress(const QString &fingerprint, const QString &desc, QString &address)
{
    LOCK(cs_ledger);
    std::string strFingerprint = fingerprint.toStdString();
    std::string strDesc = desc.toStdString();
    std::string strAddress;
    bool ret = DigiWageLedger::instance().displayAddress(strFingerprint, strDesc, strAddress);
    address = QString::fromStdString(strAddress);
    if(!ret) d->strError = QString::fromStdString(DigiWageLedger::instance().errorMessage());
    return ret;
}

QString DigiWageHwiTool::errorMessage()
{
...
bool DigiWageHwiTool::isStarted()
{
    return d->fStarted;
}

void DigiWageHwiTool::wait()
{
    if(d->fStarted)
    {
...
bool DigiWageHwiTool::rescanBlockchain(int startHeight, int stopHeight)
{
    if(!d->model) return false;
...
bool DigiWageHwiTool::importAddresses(const QString &desc)
{
    if(!d->model) return false;
...
bool DigiWageHwiTool::importMulti(const QStringList &descs)
{
    bool ret = true;
...
bool DigiWageHwiTool::finalizePsbt(const QString &psbt, QString &hexTx, bool &complete)
{
    if(!d->model) return false;
...
bool DigiWageHwiTool::sendRawTransaction(const QString &hexTx, QVariantMap& variantMap)
{
    if(!d->model) return false;
...
bool DigiWageHwiTool::decodePsbt(const QString &psbt, QString &decoded)
{
    if(!d->model) return false;
...
bool DigiWageHwiTool::getAddressDesc(const QString &address, QString &desc)
{
    if(!d->model) return false;
...
void DigiWageHwiTool::setModel(WalletModel *model)
{
    d->model = model;
}

bool DigiWageHwiTool::execRPC(ExecRPCCommand *cmd, const QMap<QString, QString> &lstParams, QVariant &result, QString &resultJson)
{
    d->strError.clear();
...
void DigiWageHwiTool::addError(const QString &error)
{
    if(d->strError != "")
        d->strError += "\n";
    d->strError += error;
...
bool DigiWageHwiTool::isDescriptorWallet()
{
    if(!d->model) return false;
    return d->model->wallet().hasDescriptors();
}

QString DigiWageHwiTool::derivationPathPKH()
{
    std::string path = DigiWageLedger::instance().derivationPath((int)OutputType::LEGACY);
    return QString::fromStdString(path);
}

QString DigiWageHwiTool::derivationPathP2SH()
{
    std::string path = DigiWageLedger::instance().derivationPath((int)OutputType::P2SH_SEGWIT);
    return QString::fromStdString(path);
}

QString DigiWageHwiTool::derivationPathBech32()
{
    std::string path = DigiWageLedger::instance().derivationPath((int)OutputType::BECH32);
    return QString::fromStdString(path);
}
