...
using namespace DigiWageLedger_NS;

class DigiWageLedgerPriv
{
public:
    DigiWageLedgerPriv()
    {
        ...
        if(!toolExists)
        {
            LogPrintf("DigiWageLedger(): HWI tool not found %s\n", toolPath);
        }
    }
...
DigiWageLedger::DigiWageLedger():
    d(0)
{
    d = new DigiWageLedgerPriv();
}
...
DigiWageLedger &DigiWageLedger::instance()
{
    static DigiWageLedger device;
    return device;
}

bool DigiWageLedger::signCoinStake(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
...
bool DigiWageLedger::signBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &vchSig)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::isConnected(const std::string &fingerprint, bool stake)
{
...
bool DigiWageLedger::enumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::signTx(const std::string &fingerprint, std::string &psbt)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::signMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &signature)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::getKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, bool descriptorwallet, std::string &desc)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::displayAddress(const std::string &fingerprint, const std::string &desc, std::string &address)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
bool DigiWageLedger::displayAddress(const std::string &fingerprint, int type, const std::string &path, std::string &address)
{
    LOCK(cs_ledger);
    // Check if tool exists
    if(!toolExists())
...
std::string DigiWageLedger::errorMessage()
{
    LOCK(cs_ledger);
...
bool DigiWageLedger::toolExists()
{
    return d->toolExists;
}

bool DigiWageLedger::isStarted()
{
    return d->fStarted;
}

void DigiWageLedger::wait()
{
    if(d->fStarted)
    {
...
bool DigiWageLedger::beginSignTx(const std::string &fingerprint, std::string &psbt)
{
...
bool DigiWageLedger::endSignTx(const std::string &, std::string &psbt)
{
...
bool DigiWageLedger::beginSignBlockHeader(const std::string &fingerprint, const std::string &header, const std::string &path, std::vector<unsigned char> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool DigiWageLedger::endSignBlockHeader(const std::string &, const std::string &, const std::string &, std::vector<unsigned char> &vchSig)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool DigiWageLedger::beginEnumerate(std::vector<LedgerDevice> &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
    arguments << "enumerate";
...
bool DigiWageLedger::endEnumerate(std::vector<LedgerDevice> &devices, bool stake)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    ...
    for(size_t i = 0; i < jsonDevices.size(); i++)
    {
        ...
        bool isStakeApp = device.app_name == "DigiWage Stake" || device.app_name == "DigiWage Stake Test";
        if(isStakeApp == stake)
        {
...
bool DigiWageLedger::beginSignMessage(const std::string &fingerprint, const std::string &message, const std::string &path, std::string &)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool DigiWageLedger::endSignMessage(const std::string &, const std::string &, const std::string &, std::string &signature)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
bool DigiWageLedger::beginGetKeyPool(const std::string &fingerprint, int type, const std::string& path, bool internal, int from, int to, bool, std::string &)
{
    // Get the output type
...
bool DigiWageLedger::endGetKeyPool(const std::string &, int type, const std::string& , bool, int, int, bool descriptorwallet, std::string &desc)
{
    // Decode command line results
...
bool DigiWageLedger::beginDisplayAddress(const std::string &fingerprint, int type, const std::string& path)
{
    // Get the output type
    std::string descType = get_address_type(type);
...
bool DigiWageLedger::beginDisplayAddress(const std::string &fingerprint, const std::string &desc)
{
    // Execute command line
    std::vector<std::string> arguments = d->arguments;
...
bool DigiWageLedger::endDisplayAddress(std::string &address)
{
    // Decode command line results
    UniValue jsonDocument = json_read_doc(d->strStdout);
    UniValue data = json_get_object(jsonDocument);
...
std::string DigiWageLedger::derivationPath(int type)
{
