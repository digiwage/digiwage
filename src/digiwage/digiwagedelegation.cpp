...
class DigiWageDelegationPriv
{
public:
    DigiWageDelegationPriv():
        m_pfDelegations(0),
    ...
    virtual ~DigiWageDelegationPriv()
    {
        if(m_pfDelegations)
...
DigiWageDelegation::DigiWageDelegation():
    priv(0)
{
    priv = new DigiWageDelegationPriv();
}
...
bool DigiWageDelegation::GetDelegation(const uint160 &address, Delegation &delegation, Chainstate& chainstate) const
{
...
bool DigiWageDelegation::VerifyDelegation(const uint160 &address, const Delegation &delegation)
{
...
bool DigiWageDelegation::FilterDelegationEvents(std::vector<DelegationEvent> &events, const IDelegationFilter &filter, ChainstateManager &chainman, int fromBlock, int toBlock, int minconf) const
{
...
std::map<uint160, Delegation> DigiWageDelegation::DelegationsFromEvents(const std::vector<DelegationEvent> &events)
{
...
void DigiWageDelegation::UpdateDelegationsFromEvents(const std::vector<DelegationEvent> &events, std::map<uint160, Delegation> &delegations)
{
...
bool DigiWageDelegation::ExistDelegationContract() const
{
    LOCK(cs_main);
...
std::string DigiWageDelegation::BytecodeRemove()
{
    return DelegationABI()["removeDelegation"].selector();
}

bool DigiWageDelegation::BytecodeAdd(const std::string &hexStaker, const int &fee, const std::vector<unsigned char> &PoD, std::string &datahex, std::string &errorMessage)
{
...
bool DigiWageDelegation::IsAddBytecode(const std::vector<unsigned char> &data)
{
...
bool DigiWageDelegation::GetUnsignedStaker(const std::vector<unsigned char> &data, std::string &hexStaker)
{
...
bool DigiWageDelegation::SetSignedStaker(std::vector<unsigned char> &data, const std::string &base64PoD)
{
    if(!IsAddBytecode(data))
        return false;
