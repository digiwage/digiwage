...
void DigiWageDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool DigiWageDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule DigiWageDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t DigiWageDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t DigiWageDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t DigiWageDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t DigiWageDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool DigiWageDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void DigiWageDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void DigiWageDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void DigiWageDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data, chainstate)[0].execRes.output;
}

void DigiWageDGP::createParamsInstance(){
...
dev::Address DigiWageDGP::getAddressForBlock(unsigned int blockHeight){
...
void DigiWageDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void DigiWageDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void DigiWageDGP::parseStorageOneUint64(uint64_t& value){
...
void DigiWageDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule DigiWageDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
