...
DigiWageState::DigiWageState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = DigiWageState::openDB(_path + "/digiwageDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

DigiWageState::DigiWageState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute DigiWageState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, DigiWageTransaction const& _t, CChain& _chain, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createDigiWageAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            digiwage::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, DigiWageTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, DigiWageTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> DigiWageState::vins() const // temp
{
...
void DigiWageState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* DigiWageState::vin(dev::Address const& _a) const
{
    return const_cast<DigiWageState*>(this)->vin(_a);
}

Vin* DigiWageState::vin(dev::Address const& _addr)
{
...
// void DigiWageState::commit(CommitBehaviour _commitBehaviour)
// {
...
void DigiWageState::kill(dev::Address _addr)
{
...
void DigiWageState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void DigiWageState::deleteAccounts(std::set<dev::Address>& addrs){
...
void DigiWageState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void DigiWageState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void DigiWageState::validateTransfersWithChangeLog(){
...
void DigiWageState::deployDelegationsContract(){
    dev::Address delegationsAddress = uintToh160(Params().GetConsensus().delegationsAddress);
    if(!DigiWageState::addressInUse(delegationsAddress)){
        DigiWageState::createContract(delegationsAddress);
        DigiWageState::setCode(delegationsAddress, bytes{fromHex(DELEGATIONS_CONTRACT_CODE)}, DigiWageState::version(delegationsAddress));
        commit(CommitBehaviour::RemoveEmptyAccounts);
        db().commit();
    }
}
