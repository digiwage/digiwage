...
bool CheckTransaction(const CTransaction& tx, TxValidationState& state, unsigned int max_coinbase_script_size)
{
    ...
    if (tx.IsCoinBase())
    {
        if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > max_coinbase_script_size)
