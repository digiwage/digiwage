...
to the DIGIWAGE GitHub release page.
...
```bash
gpg --output ${VERSION}-linux/${NAME}/digiwage-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/digiwage-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/digiwage-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/digiwage-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/digiwage-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/digiwage-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/digiwage-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@digiwage.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/digiwage-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/digiwage-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/digiwage-osx-*-build.assert
```
