...
Below are some notes on how to build DigiWage Core for Windows.

The options known to work for building DigiWage Core on Windows are:
...
Note that for WSL the DigiWage Core source path MUST be somewhere in the default mount file system, for
example /usr/src/digiwage, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\digiwage`, for example:

    make install DESTDIR=/mnt/c/workspace/digiwage
