...
- [Development guidelines](#development-guidelines)
    - [General DIGIWAGE Core](#general-digiwage-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
DIGIWAGE Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
DIGIWAGE Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/digiwage-project/digiwage/blob/master/contrib/valgrind.supp))
...
$ valgrind -v --leak-check=full src/digiwaged -printtoconsole
```
...
# A coverage report will now be accessible at `./test_digiwage.coverage/index.html`.
```

**Sanitizers**
DIGIWAGE can be compiled with various "sanitizers" enabled, which add
...
- BitcoinMiner : Generates WAGEs (if wallet is enabled).
...
However, in open source software such as DIGIWAGE Core, where everyone uses
...
pay attention to for reviewers of DIGIWAGE Core code.

General DIGIWAGE Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: DIGIWAGE Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against DIGIWAGE Core so that they can be integrated
...
file descriptors it will cause problems with DIGIWAGE's `select()` loop, because
...
- When looking at other's pull requests, it may make sense to add the following section to your `.git/config`
  file:
        ...
        [remote "upstream-pull"]
                ...
                url = git@github.com:DIGIWAGE-Project/DIGIWAGE.git
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `digiwage-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `digiwage-cli` and the GUI debug console use this table to determine how to
